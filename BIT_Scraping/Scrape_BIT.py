...
def download_band_names(states,file_to_read_citys = 'uscitiesv1.4.csv',
    ...
    It then makes a set of the band names and stored that in a csv file'''
    mc = MongoClient()
...
def store_artist_data_in_mongo():
    '''This function stores the artist data in a mongodb called artist
    It scrapes the bio and genre data from the bands in town site'''
    driver = Chrome()
    bad_bands = []
    mc = MongoClient()
    capstone_db = mc['bit_scraping']
