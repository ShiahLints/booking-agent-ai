...
class data_processer(object):
    ...
    def fit(self):
        'THIS FUNCTION POPULATES THE TABLES AND THE CONVERSION DICTIONARYS'
    ...
    def make_events_df_raw(self, artist_df, states = ['WA', 'OR','CA']):
        ...
        df3.rename({'id':'event_id', 'url':'event_url'}, axis = 'columns',\
                                                            inplace = True)
    ...
    def _pre_process_venue_names(self, events):
        ...
        events['venue_name'] = events['venue_name'].str.replace('[^\w\s]','')\
                                                                .str.strip()
    ...
    def make_venue_df(self, venues_raw):
        '''THIS FUNCTION IS USED TO MAKE THE master_venues DATAFRAME AND
        THE train_venues DATAFRAME'''

        df_raw1 = venues_raw.copy()
        df_raw1 = df_raw1[['venueidentifer','venue_name','country',\
        ...
        df_raw2 = df_raw1.groupby(['venueidentifer','venue_name'\
                                    ,'country','region','city']).\
                                    agg({'artist_genre':[list], \
                                    'artist_bio':[list],'latitude':['mean'],\
                                     'longitude':['mean']})
        ...
        if self.id_to_name is None:
            self.id_to_name = dict(zip(venue_data.venue_id,\
                                        venue_data.venueidentifer))
            self.name_to_id = dict(zip(venue_data.venueidentifer,\
                                        venue_data.venue_id))
        else:
    ...
    def _make_nans(self, x):
        '''THE DATA SET HAS NANS THAT ARNT NANE SO THIS FUNCTION SOLVES THAT'''
        if x == '':
            return None
        else:
            return x
    ...
    def _add_venue_id_to_dataframe(self, events_raw):
        '''THIS FUNCTION PUTS IDS ON A DATA FRAME USING A THE name_to_id dic
        IT IS CALLED BY make_events_df AND make_venues_df'''
        events_raw['venue_id'] = (events_raw.venueidentifer).map(self.name_to_id)
        return events_raw

    def make_events_df(self, events_raw, venues_df):
        '''CALL THIS FUNCTION ON THE EVENTS_RAW TO GET A CONVERSION
        DICTIONARYS OF venue_id TO venueidentifer AND THE REVERSE
        IT IS USE TO MAKE THE master_events_df AND THE train_events DATAFRAME'''
        events = events_raw.copy()
        events = self._add_venue_id_to_dataframe(events).copy()
        ...
        events = events.join(venues_df.set_index('venue_id'),\
                            on='venue_id', how = 'inner').copy()
        events.venue_id = events.venue_id.astype(int)
        self.events_df_raw = self._add_venue_id_to_dataframe(self.events_df_raw)
        self.events_df_raw['latitude'] = (self.events_df_raw.venue_id)\
                                            .map(self.id_to_lat)
        self.events_df_raw['longitude'] = (self.events_df_raw.venue_id)\
                                            .map(self.id_to_lng)
        self.events_df_raw = self.events_df_raw[~self.events_df_raw.latitude\
                                                        .isnull()]
        return events

    def train_test_split(self, prcnt):
        '''THIS FUNCTION SPLITS THE DATA BY DATE_TIME IT IS USED TO
        MAKE TRAINING DATA AND TEST DATA'''
        events_sorted = self.events_df_raw.sort_values(by = ['datetime']).copy()
